...
export async function GET(req) {
  try {
    ...
    const events = gigs.map((gig) => {
      // --- CHANGED: parse date & times without UTC mixing, handle cross-midnight ---
      // date expected as 'YYYY-MM-DD'
      const [Y, M, D] = String(gig.date).split('-').map(Number);

      // times like 'HH:mm'
      const [sh = '00', sm = '00'] = (gig.start || '').split(':');
      const startHour = parseInt(sh, 10);
      const startMin = parseInt(sm, 10);

      const endParts = gig.end ? gig.end.split(':') : null;
      ...
      if (endParts) {
        ...
        if (m < 0) { h -= 1; m += 60; }
        if (h < 0) { h += 24; } // cross-midnight (e.g., 23:00 -> 01:00)
...
// WORKING// // app/api/calendar/route.js
...
//       const event = {
//         title: `${gig.act}${gig.location ? `, ${gig.location}` : ''}`.trim(),
//         description: gig.comments || '',
//         location: [gig.venue, gig.location].filter(Boolean).join(', '),
//         start: eventStart,
//         productId: '//res//Calendar 1.0//EN',
